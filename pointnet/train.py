...
# with_bg: default is true
parser.add_argument('--log_dir', default='./pretrained_model', help='Log dir [default: log]')
parser.add_argument('--with_bg', default = False, help='Whether to have background or not [default: True]')
...
parser.add_argument('--num_class', type=int, default = 40, help='Number of classes to classify.')
parser.add_argument('--dataset_type', type=str, default = 'modelnet40')

parser.add_argument('--train_file', default = './modelnet40/train_files.txt', help='Location of training file')
parser.add_argument('--test_file', default = './modelnet40/test_files.txt', help='Location of test file')
...
os.system('cp ../utils/data_utils.py %s' % (LOG_DIR)) # bkp of data utils
...
if (".h5" in TRAIN_FILE):
    ...
    train_files = provider.getDataFiles(TRAIN_FILE)
    TRAIN_DATA = []
    TRAIN_LABELS = []

    for file in train_files:
        curr_x, curr_t = data_utils.load_h5(file)
        TRAIN_DATA.append(curr_x[:, :NUM_POINT, :])
        TRAIN_LABELS.append(np.squeeze(curr_t))
...
if (".h5" in TEST_FILE):
    ...
    test_files = provider.getDataFiles(TEST_FILE)
    TEST_DATA = []
    TEST_LABELS = []

    for file in train_files:
        curr_x, curr_t = data_utils.load_h5(file)
        TEST_DATA.append(curr_x[:, :NUM_POINT, :])
        TEST_LABELS.append(np.squeeze(curr_t))
...
def train():
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if epoch % 10 == 0:
                save_path = saver.save(sess, os.path.join(LOG_DIR, DATA_TYPE+"_model.ckpt"))
